...
public class UserService {
	...
	public void createMailUsers(UserForCreate user, String bo, String parentBo) {
		...
		System.out.println("parentBo::::::::::: "+parentBo);
	...
	private Name buildDn(UserForCreate user, String bo, String parentBo) {
		return LdapNameBuilder.newInstance(ldapProperties.getBaseDN()).add("o", "nic.in").add("o",
				parentBo).add("o",bo).add("ou", "People").add("uid", user.getUsername()).build();
...
//			ldapTemplate.rename(dn.toString(), dataToBeUpdated.getDn());
//		}
		ldapTemplate.modifyAttributes(context);
	}
	...
	public List<Po> fetchBosFromPo(String baseDn) {
		return ldapTemplate.find(query().base(baseDn).where("objectclass")
				.is("sunDelegatedOrganization"), Po.class);
	}
	...
	public List<Po1> fetchPos(String baseDn) {
		return ldapTemplate.find(query().base(baseDn).where("objectclass")
				.is("sunmanagedprovider"), Po1.class);
	}
	...
	@HystrixCommand(fallbackMethod = "fallback_enableSmtp")
	public void enableSmtp(@NotBlank String uid) {
		UserForSearch user = findByUid(uid);
		...
		if (services.contains("-smtp:ALL")) {
			services.remove("-smtp:ALL");
		}
		if (services.contains("-smtps:ALL")) {
			services.remove("-smtps:ALL");
		}
		if (!services.contains("+smtp:ALL") && !services.contains("smtp:ALL")) {
			services.add("+smtp:ALL");
		}
		if (!services.contains("+smtps:ALL") && !services.contains("smtps:ALL")) {
			services.add("+smtps:ALL");
		}
		String finalString = "";
	...
	private void fallback_enableSmtp(String uid) {
		log.warn("CALLING fallback_enableSmtp for uid: {}",uid);
	}

	@HystrixCommand(fallbackMethod = "fallback_disableSmtp")
	public void disableSmtp(@NotBlank String uid) {
		UserForSearch user = findByUid(uid);
		String allowedServices = user.getMailallowedserviceaccess();
		List<String> services = new ArrayList<String>(Arrays.asList(allowedServices.split("\\$")));
		if (services.contains("+smtp:ALL")) {
	...
	private void fallback_disableSmtp(String uid) {
		log.warn("CALLING fallback_disableSmtp for uid : {}",uid);
	}

	@HystrixCommand(fallbackMethod = "fallback_enableSmime")
	public void enableSmime(@NotBlank String uid) {
		UserForSearch user = findByUid(uid);
		String allowedServices = user.getMailallowedserviceaccess();
		List<String> services = new ArrayList<String>(Arrays.asList(allowedServices.split("\\$")));
		if (services.contains("-smime:ALL")) {
			services.remove("-smime:ALL");
		}
		if (!services.contains("+smime:ALL") && !services.contains("smime:ALL")) {
			services.add("+smime:ALL");
		}
		String finalString = "";
		for (String string : services) {
			finalString += string + "$";
	...
	private void fallback_enableSmime(String uid) {
		log.warn("CALLING fallback_enableSmime for uid: {}",uid);
	}

	@HystrixCommand(fallbackMethod = "fallback_disableSmime")
	public void disableSmime(@NotBlank String uid) {
		UserForSearch user = findByUid(uid);
		String allowedServices = user.getMailallowedserviceaccess();
		List<String> services = new ArrayList<String>(Arrays.asList(allowedServices.split("\\$")));
		if (services.contains("+smime:ALL")) {
	...
	private void fallback_disableSmime(String uid) {
		log.warn("CALLING fallback_disableSmime for uid : {}",uid);
	}
	...
	public void enableImap(@NotBlank String uid) {
		...
		if (services.contains("-imap:ALL")) {
			services.remove("-imap:ALL");
		}
		if (!services.contains("+imap:ALL") && !services.contains("imap:ALL")) {
			services.add("+imap:ALL");
		}
		String finalString = "";
		for (String string : services) {
			finalString += string + "$";
	...
	public void disablePop(@NotBlank String uid) {
		...
		if (services.contains("+pop:ALL")) {
			services.remove("+pop:ALL");
		} else {
			if (!services.contains("-pop:ALL") && services.contains("pop:ALL")) {
				services.remove("pop:ALL");
			}
		}
		if (!services.contains("-pop:ALL")) {
			services.add("-pop:ALL");
		}
		String finalString = "";
		for (String string : services) {
			finalString += string + "$";
	...
	public AssociatedDomains fetchAssociatedDomains() {
		List<AssociatedDomains> associatedDomains = ldapTemplate.find(query().base(ldapProperties.getBaseDN()).where("associateddomain").like("*"), AssociatedDomains.class);
