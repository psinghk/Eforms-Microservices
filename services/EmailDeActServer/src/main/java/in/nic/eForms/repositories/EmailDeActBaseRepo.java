...
@Repository
public interface EmailDeActBaseRepo extends JpaRepository<EmailDeActBase, Long>{
	
	EmailDeActBase findByRegistrationNo(String registrationNo);
	List<EmailDeActBase> findByDatetimeLike(String datetime);
	

	@Query(value = "select registration_no from email_deact_registration where date(datetime)=date(now()) group by registration_no desc limit 1", nativeQuery = true)
