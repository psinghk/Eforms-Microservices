...
import in.nic.ashwini.eForms.entities.CreateUserAuth;
import in.nic.ashwini.eForms.entities.DaAppAuth;
import in.nic.ashwini.eForms.entities.SupportAppAuth;
import in.nic.ashwini.eForms.repositories.CreateUserAuthRepository;
import in.nic.ashwini.eForms.repositories.DaAppAuthRepository;
import in.nic.ashwini.eForms.repositories.SupportAppRepo;
...
public class RolesService {
	...
	@Autowired
	private CreateUserAuthRepository createUserAuthRepository;

	@Autowired
	private SupportAppRepo supportAppRepo;



	@Autowired
	private DaAppAuthRepository daAppAuthRepository;
	...
	public Set<String> fetchRoles(Set<String> aliases, String email, String mobile, boolean govEmployee) {
		...
		if (govEmployee) {
			...
			List<CreateUserAuth> list = createUserAuthRepository.findByEmailInAndIpAndStatus(aliases, remoteIp, 0);

			if(list != null) {
				roles.add("ROLE_CREATE_USER_APP");
			}

			List<SupportAppAuth> slist = supportAppRepo.findByEmailInAndIpAndStatus(aliases, remoteIp, 0);
			System.out.println("inside role assign");
			if(slist != null) {
				roles.add("ROLE_SUPPORT_APP");
			}
			List<DaAppAuth> daList = daAppAuthRepository.findByEmailInAndIpAndStatus(aliases, remoteIp, 0);
