	...
	protected OAuth2Authentication getOAuth2Authentication(ClientDetails client, TokenRequest tokenRequest) {
		...
		if (mfaToken != null) {
			...
			if (!parameters.containsKey("otp_for")) {
				...
				throw new InvalidRequestException("otp_for is missing!!!");
			}
			...
			if (parameters.get("otp_type").equals("resend")) {
				if (parameters.get("otp_for").equals("email")) {
					Optional<EmailOtp> emailOtpList = emailOtpRepository.findTopByEmailAndGenerationTimeStampBeforeAndExpiryTimeStampAfterOrderByIdDesc(usernameDecrypted, LocalDateTime.now(), LocalDateTime.now());
					...
					Optional<MobileOtp> mobileOtpList = mobileOtpRepository
