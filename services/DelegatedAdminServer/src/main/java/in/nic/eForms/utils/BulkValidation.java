...
//                }
	...
	/*
	 * Author : Sachin Malik, 
	 * Mehtod Working : Check Date Of Retirement is less then 60 year or not, 
	 * Date : 21-09-2021  
	 */
	public String dorValidation(String dobDate, String dorDate) throws Exception{
        ...
        String minDOB = new SimpleDateFormat("dd-MM-yyyy").format(date2);
        //System.out.println(minDOB);
         try{
         Date minDOBdate = new SimpleDateFormat("dd-MM-yyyy").parse(minDOB); 
         Date userDOBdate = new SimpleDateFormat("dd-MM-yyyy").parse(dobDate);
        
		if(minDOBdate.compareTo(userDOBdate) > 0) {
             flag = true;
        } else if(minDOBdate.compareTo(userDOBdate) < 0) {
             return "User age should be greater then 18 year.";
        } else if(minDOBdate.compareTo(userDOBdate) == 0) {
             flag = true;
        }
        }
		...
		if(dobDate==null && dobDate=="") {
			msg = "Date of birth is not avilable. please update date of birth first";
		} else if(dorDate==null && dorDate=="") {
			msg = "Account expiry date cannot be blank";    			
		} 
		else if(flag) {
			...
			try {
				sdf2.parse(dobDate);
				sdf2.parse(dorDate);    /* 1. Check valid date format */
				
					String dateInString = new SimpleDateFormat("dd-MM-yyyy").format(date);
				
				Date dor = new SimpleDateFormat("dd-MM-yyyy").parse(dorDate);
				if (date.compareTo(dor) > 0) {                /* Check date of retirement is future date on not */
					msg = "Account Expiry date Must be greater then present date.";
				} else {
					// CONVERT STRING DATE INTO DATE DATA TYPE;
					Date dobDateTemp = new SimpleDateFormat("dd-MM-yyyy").parse(dobDate);
					Date userEnterDorDate = new SimpleDateFormat("dd-MM-yyyy").parse(dorDate);

					Calendar c = Calendar.getInstance();
					c.setTime(dobDateTemp);
					c.add(Calendar.YEAR, 60);
					Date fixedDor = c.getTime();
					String strFixedDOR = formatter.format(fixedDor);

					SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
					SimpleDateFormat sdf3 = new SimpleDateFormat("MM-dd-yyyy");
					sdf3.format(sdf.parse(strFixedDOR));

					int dorMonth = Integer.parseInt(strFixedDOR.split("-")[1]); // get month in date.
					int dorYear = Integer.parseInt(strFixedDOR.split("-")[2]); // get year in date.

					// Get the number of days in that month
					YearMonth yearMonthObject = YearMonth.of(dorYear, dorMonth);
					int daysInMonth = yearMonthObject.lengthOfMonth(); // get days in months.
					System.out.println("DAYS IN MONTH ::::: " + daysInMonth+" MONTH ::::: "+yearMonthObject);
					strFixedDOR = daysInMonth + "-" + dorMonth + "-" + dorYear; // fixed date of retirement according to dob.
				
					Date RetirementDate = new SimpleDateFormat("dd-MM-yyyy").parse(strFixedDOR);

					if (userEnterDorDate.compareTo(RetirementDate) > 0) {
						msg = "Account expiry date can not exceed 60 years.";
					} else if (userEnterDorDate.compareTo(RetirementDate) < 0) {
				...
				System.out.println("PARSE EXCEPTION ERROR ::::: " + e);
				msg = "Account Expiry date is not valid [eg:DD-MM-YYYY]";
			}
		}
		return msg;
}	
	...
	/* 
	 * Author : Sachin Malik, 
	 * Method Working : Format DOR to display from LDAP, 
	 * Return : String date like DD-MM-YYYY format,
	 */
	/*  */
	public static String formatDateToShow(String dt) {
		String finalDate = "";
		String newDate = dt.replace("Z", "");
		try {
			java.util.Date date1 = new SimpleDateFormat("yyyyMMddHHmmss").parse(newDate);
	...
	/* 
	 * Author : Sachin Malik, 
	 * Method Working : Format DOR to save in LDAP, 
	 * Return : String date like yyyyMMdd000000Z format,
	 */
	public static String formatDateToSave(String dt) {
		String finalDate = "";
		try {
			java.util.Date date = new SimpleDateFormat("dd-MM-yyyy").parse(dt);
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
			finalDate = (sdf.format(date) + "000000Z").trim();
		} catch (ParseException ex) {
			ex.printStackTrace();
		}
		return finalDate;
	...
	/* 
	 * Author : Sachin Malik, 
	 * Method Working : Generate Random Password, 
	 * Return : Random Password in String type,
	 */
