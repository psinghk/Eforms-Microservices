...
public class EmailActService {
	...
	@Autowired
	public EmailActService(GeneratePdfRepository generatePdfRepository, EmailActBaseRepo emailActBaseRepo, Util utilityService) {
		super();
	...
	public JasperPrint generateFormPdf(String regid)
		...
		if (formDetails.isPresent()) {
			...
			parameters.put("Dor", generatePdfBean.getDor());
			parameters.put("WorkOrder", generatePdfBean.getWorkrOrder());
	...
	public boolean updatePreviewDetails(String regNumber, PreviewFormBean previewFormBean) {
		EmailActBase emailActBase = emailActBaseRepo.findByRegistrationNo(regNumber);
		...
		if (emailActBase != null) {
			try {
				...
				if (previewFormBean.getWorkrOrder() != null) {
					System.out.println("WorkrOrder:::::::::::" + previewFormBean.getWorkrOrder().getOriginalFilename());
					emailActBase.setWorkrOrder(previewFormBean.getWorkrOrder().getOriginalFilename());
					//emailActBase.setPdfPath(Constants.LOCAL_FILE_LOCATION + previewFormBean.getWorkrOrder().getOriginalFilename());
					byte[] bytes = previewFormBean.getWorkrOrder().getBytes();
					Path path = Paths.get(Constants.LOCAL_FILE_LOCATION + previewFormBean.getWorkrOrder().getOriginalFilename());

					Files.write(path, bytes);
				}
				EmailActBase emailActBase1 = emailActBaseRepo.save(emailActBase);
