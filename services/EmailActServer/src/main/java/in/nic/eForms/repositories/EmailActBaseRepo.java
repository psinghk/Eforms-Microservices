...
@Repository
public interface EmailActBaseRepo extends JpaRepository<EmailActBase, Long>{
	
	EmailActBase findByRegistrationNo(String registrationNo);
	List<EmailActBase> findByDatetimeLike(String datetime);
	

	@Query(value = "select registration_no from email_act_registration where date(datetime)=date(now()) group by registration_no desc limit 1", nativeQuery = true)
