...
public class VpnService {
	...
	public VpnBase insert(VpnBase vpnBase) {
		if (vpnBase != null) {
			...
			String newRegNumber = "VPN-FORM" + currentDate;
			if (oldRegNumber == null || oldRegNumber.isEmpty()) {
				newRegNumber += "0001";
			} else {
				int newRegistrationNo = Integer.parseInt(oldRegNumber.substring(16, oldRegNumber.length())) + 1;
	...
	public boolean updatePreviewDetails(String regNumber, PreviewFormBean previewFormBean) {
		...
		if (vpnBase != null) {
			try {
				...
				VpnBase vpnBaseUpdated = vpnBaseRepo.save(vpnBase);
				if (vpnBaseUpdated.getId() > 0) {
					return true;
				}
			} catch (Exception e) {
				return false;
	...
	@Transactional
	public String insertIntoEntry(String regno, List<FormData> value) {
		try {
