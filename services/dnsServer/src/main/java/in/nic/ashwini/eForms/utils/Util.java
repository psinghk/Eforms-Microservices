...
public class Util {
	...
	public Boolean isNicEmployee(String mail) {
		...
		HttpEntity<Boolean> response = restTemplate.exchange(builder.toUriString(), HttpMethod.POST, entity,
				Boolean.class);
		log.info("{} is NIC Employee : {}",mail,response.getBody().booleanValue());
		return response.getBody();
	}
	...
	public List<String> fetchCompletedRegNumbers(List<String> regNumber) {
		...
		headers.setContentType(MediaType.APPLICATION_JSON);
		headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
		RegNumberBeanForDns regNumberBean = new RegNumberBeanForDns();
		regNumberBean.setRegNumbers(regNumber);
		HttpEntity<RegNumberBeanForDns> requestEntity = new HttpEntity<RegNumberBeanForDns>(regNumberBean,headers);
		String uri = REPORTING_URL+"/fetchCompletedRegNumbers";
	...
	public Boolean updateStatusAndFinalAuditTrack(Status status, FinalAuditTrack finalAuditTrack) {
		...
		log.info("Update of status & final_audit_track table response is {} ",response.getBody().toString());
