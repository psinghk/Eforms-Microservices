package in.nic.eform.smsip.service;
...
@Service
public class SmsIPService {
	private static final Logger log = LoggerFactory.getLogger(SmsIPService.class);
	@Value("${fileBasePath}")
	...
	//********************************Start of Submission******************************
		public Map<String, Object> submissionPreview(SmsIPFormBean smsIPFormBean)  {
			log.info("Inside submissionPreview method at " + new Date() + " submissionSmsFormBean:::" +smsIPFormBean );
			 ...
			 try {
				BeanUtils.populate(smsIPFormBean, profileList);
			...
			ldapBeanList = m.convertValue(smsIPFormBean, Map.class);
...
//			}
			...
			public Map<String, Object> generateRefNo(SmsIPFormBean smsIPFormBean)  {
				 log.info("Inside generateRefNo method at " + new Date() +" submissionSmsFormBean:::"+smsIPFormBean);
			 ...
			 SmsIPBaseDTO smsIPBaseDTO =new SmsIPBaseDTO();
			 try {
				BeanUtils.copyProperties(smsIPBaseDTO, smsIPFormBean);
			} catch (IllegalAccessException e) {
				e.printStackTrace();
			} catch (InvocationTargetException e) {
				e.printStackTrace();
			 ...
			 String ref_num = smsIPDao.ldap_tab2(smsIPBaseDTO);
			 log.info("Inside generateRefNo method at " + new Date() +" ref_num:::"+ref_num);
...
//		               String isHod=isHodApi(email);
		            ...
		            if (!check.equals("esign")) {
		            	...
		            	if(!map.isEmpty()) {
		            		 ...
		            		 consentreturn.put("ref_num", ref_num);
		 	  	            consentreturn.put("status", "Something went Wrong!!");
		            	}
			...
			public Map<String, String> finalSubmission(ForwardBean forwardBean) {
				  log.info("Inside finalSubmission method at " + new Date() +" submissionBean:::"+forwardBean);
				Map<String, String> form_details = null;
				Map<String, String> forwardToData = new HashMap<>();
				form_details = smsIPDao.fetchFormDetail(forwardBean.getRef_num());
					...
					if (submissionType.equals("manual_upload")) {
						...
						if (form_details != null) {
							if (isNicEmployee) {
								...
								forwardToData.put("toemail", smsIPDao.fetchToEmail());
							} else if (employment.equalsIgnoreCase("state") && state.equalsIgnoreCase("Himachal Pradesh")) {
								Map<String, String> himachalCoordsAsRO = smsIPDao
			...
			public boolean updateUsrStatus(String ref_num, String role, String submissionType, String statRemarks, String file,
				...
				log.info("Inside SubmissionSmsService: updateStatus method at " + new Date());
				...
				log.info("Inside SubmissionSmsService: updateStatus method at " + new Date()+" status:::"+status);
				return status;
			}

			
			public int insertTrackTable(Map<String, String> forwardBydata, Map<String, String> forwardToData) {
				log.info("Inside SubmissionSmsService: updateTrackTable method at " + new Date());
				String role = forwardBydata.get("role");
				int i = 0, j = 0, k = 0;
				log.info("Inside SubmissionSmsService: updateTrackTable method at " + new Date()+" role:::"+role);
				if (role.equals(Constants.ROLE_USER)) {
					i = smsIPDao.insertIntoAppType(forwardBydata, forwardToData);
					if (forwardBydata.get("check").equalsIgnoreCase("upload_scanned")) {
						j = smsIPDao.updateFinalAuditTrack(forwardBydata, forwardToData);
					} else {
						j = smsIPDao.insertIntoFinalAuditTrack(forwardBydata, forwardToData);
					}
				}  else {
					j = smsIPDao.updateFinalAuditTrack(forwardBydata, forwardToData);
				}
					if (i > 0 && j > 0) {
						k = smsIPDao.updateStatusTable(forwardBydata, forwardToData);
					}
					log.info("Inside SubmissionSmsService: updateTrackTable method at " + new Date()+" k:::"+k);
				return k;
			}
	//********************************End of Submission********************************
	...
	public Map<String, Object> preview(String regno,String role,String applicant_email) {
		...
		Set<String> fetchAliases =new HashSet<String>(apiUtility.fetchAliases(applicant_email));
		 ...
		 boolean isEditable = smsIPDao.checkStatusForPreview(regno, applicant_email, role, "toPreview", fetchAliases);
		 log.info("Inside preview method at  " + new Date() +" isEditable:::"+isEditable);
		  if (isEditable) {
			  map=smsIPDao.preview(regno);
		  }
	...
	public boolean validateRefNo(String refNo) {
		return smsIPDao.validateRefNo(refNo);
	}
	...
	public Map<String, String> approve(ForwardBean forwardBean) {
		...
		fetchToEmail=smsIPDao.fetchToEmail();
              ...
              //  }
                ...
                if (form_details != null) {
                    if (isNicEmployee) {
                        ...
                        Map<String, String> himachalCoordsAsRO = smsIPDao.fetchHimachalCoord(form_details.get("department"));
                        forwardToData.put("nextlevel", Constants.ROLE_CA);
                        forwardToData.put("toemail", hod_email);
                        if (himachalCoordsAsRO.size() > 0) {
                            String userForSearch = apiUtility.findMobilebyId(himachalCoordsAsRO.get("email"));
...
//                }
        ...
        case Constants.ROLE_CA:
			...
			String coemails = smsIPDao.convertSetToString(coordinators);
            if (isNicEmployee) {
        ...
        case Constants.ROLE_SUP:
            ...
            if (adminType.equals("da")) {
                correctRecpEmails = smsIPDao.isRecipientDa(toEmails,fetchAliases1);
                if (!correctRecpEmails.isEmpty()) {
                    correctRecpEmailsInString = smsIPDao.convertSetToString(correctRecpEmails);
                ...
                correctRecpEmails = smsIPDao.isRecipientAdmin(toEmails);
                if (!correctRecpEmails.isEmpty()) {
                    correctRecpEmailsInString = smsIPDao.convertSetToString(correctRecpEmails);
                    forwardToData.put("nextlevel", Constants.ROLE_MAILADMIN);
                } else {
            	...
            	correctRecpEmails = smsIPDao.isRecipientCoordinator(toEmails,fetchAliases1);
                if (!correctRecpEmails.isEmpty()) {
                    correctRecpEmailsInString = smsIPDao.convertSetToString(correctRecpEmails);
                    if (correctRecpEmails.contains("support@gov.in") || correctRecpEmails.contains("support@nic.in") || correctRecpEmails.contains("support@dummy.nic.in")) {
                        forwardToData.put("nextlevel", Constants.ROLE_MAILADMIN);
	...
	public Map<String, Object> fetchCoordinators(Map<String, String> form_details, String role, String uemail,
		...
		if (form_details.get("applicant_employment").equalsIgnoreCase("state")
			...
			punjabCoords = smsIPDao.fetchPunjabNodalOfficers(form_details.get("applicant_posting_city"));
			punjabDA = smsIPDao.fetchPunjabDA(form_details.get("applicant_employment"), form_details.get("applicant_ministry"),
					form_details.get("applicant_department"));
			empCoords.put("coordinators", smsIPDao.convertStringToSet(punjabCoords));
			empCoords.put("daEmail", punjabDA);
			empCoords.put("isDaAvailable", true);
		} else {
			fetchcoords = smsIPDao.fetchEmailDetails(form_details.get("applicant_employment"), form_details.get("applicant_ministry"),
					form_details.get("applicant_department"));
			...
			Set<String> finalCoords = smsIPDao.removeSupportFromSet(coordinators);

			// remove support id from admins
			Set<String> finalAdmins = smsIPDao.removeSupportFromSet(admins);
			...
			if (!finalCoords.isEmpty() && !finalAdmins.isEmpty()) {
				if (finalAdmins.equals(finalCoords)) {
					empCoords.put("isDaAvailable", true);
					empCoords.put("daEmail", smsIPDao.convertSetToString(finalAdmins));
				} else {
					...
					if (form_details.get("applicant_employment").equalsIgnoreCase("state")
						...
						if (flagForDA) {
							empCoords.put("isDaAvailable", true);
							empCoords.put("daEmail", smsIPDao.convertSetToString(da));
						}
					}
				}
			} else {
		...
		public boolean reject(String reg_no, String role, String statRemarks, String check, String email,
			...
			if (smsIPDao.updateTrackTable(forwardBydata, forwardToData)) {
				status = true;
			}
		...
		public HashMap<String, Object> fetchTrackDetails(String reg_no) {
			...
			fetchDaoList=smsIPDao.fetchTrackDetails(reg_no);
			log.info("Inside fetchTrackDetails method at " + new Date() +" fetchDaoList:::"+fetchDaoList);
			List<SmsIPStatusDTO> arrRoles = (List<SmsIPStatusDTO>) fetchDaoList.get("arrRoles");
			...
			List<SmsIPFinalAuditTrackDTO> currentDetails = (List<SmsIPFinalAuditTrackDTO>) fetchDaoList.get("currentDetails");
		...
		public Map<String, Object> fetchTrackByRole(String reg_no, String forward, String trole, String srole) {
			...
			Map<String, Object> getDaoVal = smsIPDao.fetchTrackByRole(reg_no, forward, trole, srole);
		...
		public Map<String, String> validateRoles(List<SmsIPStatusDTO> arrRoles){
			...
			for (SmsIPStatusDTO trackStatus : arrRoles) {
		...
		public Map<String, String> validateDetails(List<SmsIPFinalAuditTrackDTO> currentDetails){
			...
			for (SmsIPFinalAuditTrackDTO trackdto : currentDetails) {
		...
		public  Map<String, Object> raiseQuery(String regid, String role, String uemail,String choose_recp,String to_email,String statRemarks) {
			...
			Map<String, Object> map=null;
			map = smsIPDao.raiseQuery(regid, role, uemail,choose_recp,to_email,statRemarks);
		...
		public ArrayList<String> fetchRecps(String regNo, String role) {
			...
			map=smsIPDao.fetchRaiseQueryData(regNo,role);
		...
		public Map<String, Object> putOnHold(String regNo, String type, String role, String statRemarks) {
			...
			Map<String, Object> hmTrack = new HashMap<>();
			 ...
			 flag = smsIPDao.putOnHold( regNo,  on_hold,  statRemarks);
	         if (flag) {
		...
		public JasperPrint generatePdf(String regid) throws JRException, IOException {
			...
			SmsIPBaseDTO smsIPBaseDTO = smsIPDao.findByRegno(regid);
			if (smsIPBaseDTO!=null) {
				GeneratePdfBean generatePdfBean = new GeneratePdfBean();
				try {
					BeanUtils.copyProperties(smsIPBaseDTO, generatePdfBean);
				} catch (IllegalAccessException e) {
					e.printStackTrace();
				} catch (InvocationTargetException e) {
					e.printStackTrace();
			...
			for (MultipartFile f : infile) {
				if (!f.isEmpty()) {
					...
					try {
						...
						SmsIPDocUploadDTO docUploadDTO = new SmsIPDocUploadDTO();
						BeanUtils.copyProperties(uploadfiles, docUploadDTO);
						docUploadDTO.setDoc(outputfile);
						docUploadDTO.setDocpath(fileBasePath);
						docUploadDTO.setExtension(ext);
						docUploadDTO.setOriginal_filename(f.getOriginalFilename());
						docUploadDTO.setRegistrationno(registrationno);
						docUploadDTO.setRole(role);
						docUploadDTO = smsIPDao.saveDocx(docUploadDTO);
						log.info("Entering saveDocx method at info " + new Date() +" id::::"+docUploadDTO.getId());
						if (docUploadDTO.getId() == 0) {
		...
		public Map<String, Map<String, String>> viewDocx(String regid, String role) {
			...
			case "sup":
				alldocx = smsIPDao.viewDocx(regid, "sup");
				alldocx = smsIPDao.viewDocx(regid, "admin");
				alldocx = smsIPDao.viewDocx(regid, "da");
			case "admin":
			case "da":
				if (!role.equals("sup")) {
					  if (role.equals("da")) {
						  smsIPDao.viewDocx(regid, "da");
		              } else {
		            	  smsIPDao.viewDocx(regid, "admin");
		              }
					  files.putAll(alldocx);
				}
			case "co":
				alldocx = smsIPDao.viewDocx(regid, "co");
				files.putAll(alldocx);
			case "ca":
				alldocx = smsIPDao.viewDocx(regid, "ca");
				files.putAll(alldocx);
			case "user":
				alldocx = smsIPDao.viewDocx(regid, "user");
				files.putAll(alldocx);
				break;
			}
