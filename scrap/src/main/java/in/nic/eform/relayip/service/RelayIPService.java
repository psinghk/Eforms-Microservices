package in.nic.eform.relayip.service;
...
@Service
public class RelayIPService {
	private static final Logger log = LoggerFactory.getLogger(RelayIPService.class);
	@Value("${fileBasePath}")
	private String EXTERNAL_FILE_PATH;
	@Value("${fileBasePath}")
	private String fileBasePath;
	@Autowired
	RelayIPDao relayIPDao;
	@Autowired
	...
	//********************************Start of Submission******************************
		public Map<String, Object> submissionPreview(RelayIPFormBean relayIPFormBean)  {
			log.info("Entering submissionPreview method at info " + new Date() +" submissionRelayFormBean::::"+relayIPFormBean);
			 ...
			 try {
				BeanUtils.populate(relayIPFormBean, profileList);
			...
			ldapBeanList = m.convertValue(relayIPFormBean, Map.class);
			log.info("Entering submissionPreview method at info " + new Date() +" ldapBeanList::::"+ldapBeanList);
		return ldapBeanList;
		}
		
		
			public Map<String, Object> submissionRoDetails()  {
				log.info("Entering submissionRoDetails method at info " + new Date());
				 Map<String, Object> ldapBeanList = new HashMap<String, Object>();
				 HashMap<String, Object> profileList = new HashMap<String, Object>();
				 profileList=getProfileInfo.fetchprofile();
				 ...
				 log.info("Entering submissionRoDetails method at info " + new Date()+" ldapBeanList:::"+ldapBeanList);
			return ldapBeanList;
			}
		
		
			public Map<String, Object> generateRefNo(RelayIPFormBean relayIPFormBean)  {
				 log.info("Entering generateRefNo method at info " + new Date()+" submissionRelayFormBean:::"+relayIPFormBean);
			 ...
			 RelayIPBaseDTO relayIPBaseDTO =new RelayIPBaseDTO();
			 try {
				BeanUtils.copyProperties(relayIPBaseDTO, relayIPFormBean);
			} catch (IllegalAccessException e) {
				e.printStackTrace();
			} catch (InvocationTargetException e) {
				e.printStackTrace();
			 ...
			 String ref_num = relayIPDao.ldap_tab2(relayIPBaseDTO);
			 log.info("Entering generateRefNo method at info " + new Date()+" ref_num:::"+ref_num);
			 // int i = userSubmissionDao.update_profile(ldapFormDto);
			  ...
			  log.info("Entering generateRefNo method at info " + new Date()+" map:::"+map);
			return map;
			}
			...
			public Map<String, Object> consent(String regno,String check,String formtype,String consent,String email) {
				 log.info("Entering consent method at info " + new Date()+" regno:::"+regno+"  check:::"+check+"  formtype:::"+formtype+"  consent:::"+consent+" email:::"+email);
				Map<String, Object> map = new HashMap<String, Object>();
...
//		               String isHod=isHodApi(email);
			...
			public Map<String, String> finalSubmission(ForwardBean forwardBean) {
				log.info("Entering finalSubmission method at info " + new Date()+" submissionBean:::"+forwardBean);
				Map<String, String> form_details = null;
				Map<String, String> forwardToData = new HashMap<>();
				form_details = relayIPDao.fetchFormDetail(forwardBean.getRef_num());
				log.info("Entering finalSubmission method at info " + new Date()+" form_details:::"+form_details);
				...
				log.info("Entering finalSubmission method at info " + new Date()+" isNicEmployee:::"+isNicEmployee);
				log.info("Entering finalSubmission method at info " + new Date()+" employment:::"+employment+"  state:::"+state);
				log.info("Entering finalSubmission method at info " + new Date()+" currentLevel:::"+currentLevel);
					...
					if (submissionType.equals("manual_upload")) {
						...
						if (form_details != null) {
							if (isNicEmployee) {
								...
								forwardToData.put("toemail", relayIPDao.fetchToEmail());
							} else if (employment.equalsIgnoreCase("state") && state.equalsIgnoreCase("Himachal Pradesh")) {
								Map<String, String> himachalCoordsAsRO = relayIPDao
			...
			public boolean updateUsrStatus(String ref_num, String role, String submissionType, String statRemarks, String file,
				...
				log.info("Entering updateStatus method at info " + new Date());
				...
				log.info("Entering updateStatus method at info " + new Date()+" status:::"+status);
				return status;
			}
			...
			public int insertTrackTable(Map<String, String> forwardBydata, Map<String, String> forwardToData) {
				...
				log.info("Entering updateTrackTable method at info " + new Date()+" role:::"+role);
				int i = 0, j = 0, k = 0;
				if (role.equals(Constants.ROLE_USER)) {
					i = relayIPDao.insertIntoAppType(forwardBydata, forwardToData);
					if (forwardBydata.get("check").equalsIgnoreCase("upload_scanned")) {
						j = relayIPDao.updateFinalAuditTrack(forwardBydata, forwardToData);
					} else {
						j = relayIPDao.insertIntoFinalAuditTrack(forwardBydata, forwardToData);
					}
					log.info("Entering updateTrackTable method at info " + new Date()+" j:::"+j);
				}  else {
					j = relayIPDao.updateFinalAuditTrack(forwardBydata, forwardToData);
				}
					if (i > 0 && j > 0) {
						k = relayIPDao.updateStatusTable(forwardBydata, forwardToData);
					}
					log.info("Entering updateTrackTable method at info " + new Date()+" k:::"+k);
				return k;
			}
	...
	//********************************Start of Preview(after submission)******************
			public Map<String, Object> preview(String regno,String role,String applicant_email) {
				...
				Set<String> fetchAliases =new HashSet<String>(apiUtility.fetchAliases(applicant_email));
				 ...
				 boolean isEditable = relayIPDao.checkStatusForPreview(regno, applicant_email, role, "toPreview", fetchAliases);
				 log.info("Inside preview method at  " + new Date() +" isEditable:::"+isEditable);
				  if (isEditable) {
					  map=relayIPDao.preview(regno);
				  }
			...
			public Map<String, Object> EditpreviewDetails(String regno,String role,String applicant_email) {
				...
				Set<String> fetchAliases =new HashSet<String>(apiUtility.fetchAliases(applicant_email));
				 ...
				 boolean isEditable = relayIPDao.checkStatusForEdit(regno, applicant_email, role, "toEdit", fetchAliases);
				 log.info("Inside EditpreviewDetails method at  " + new Date()+" isEditable:::"+isEditable);
				  if (isEditable) {
					  map= relayIPDao.EditPreview(regno);
				  }
				  log.info("Inside EditpreviewDetails method at  " + new Date()+" map:::"+map);
				  return map;
			}
			
			public Map<String, Object> UpdatePreviewDetails(RelayIPFormBean relayIPFormBean) {
				 log.info("Inside UpdatePreviewDetails method at  " + new Date());
				Map<String, Object> map=new HashMap<String, Object>();
			boolean status=false;
			status= relayIPDao.UpdatePreviewDetails(relayIPFormBean);
			log.info("Inside UpdatePreviewDetails method at  " + new Date()+"  status:::"+status);
			if(status) {
					  map.put("status", "Preview Updated successfully!!");
					  map.put("RegistrationNo", relayIPFormBean.getRegistrationno());
				  }else {
					  map.put("status", "Something went wrong!!");
					  map.put("RegistrationNo", relayIPFormBean.getRegistrationno());
			}
	...
	//********************************Start of Approve/Forward******************************
			public boolean validateRefNo(String refNo) {
				return relayIPDao.validateRefNo(refNo);
			}
			...
			public Map<String, String> approve(ForwardBean forwardBean) {
				...
				fetchToEmail=relayIPDao.fetchToEmail();
				log.info("Inside approve method at " + new Date()+" fetchToEmail:::"+fetchToEmail);
				forwardToData.put("currentlevel", currentLevel);
				String ipRequestWhichType = form_details.get("ip_action_request").toString();

			    switch (currentLevel) {
		        case Constants.ROLE_USER:
		            if (submissionType.equals("online") || submissionType.contains("esign")) {
		               // if (formtype.equals("ip") ||formtype.equals("bulk")&& employment.equals(state) && state.equalsIgnoreCase("himachal pradesh")) {
		                    forwardToData.put("nextlevel", Constants.ROLE_CA);
		                    forwardToData.put("toemail", hod_email);
		                //}
		            } 
		                ...
		                if (form_details != null) {
		                    if (isNicEmployee) {
		                        ...
		                        Map<String, String> himachalCoordsAsRO = relayIPDao.fetchHimachalCoord(form_details.get("department"));
		                        forwardToData.put("nextlevel", Constants.ROLE_CA);
		                        forwardToData.put("toemail", hod_email);
		                        if (himachalCoordsAsRO.size() > 0) {
		                            String userForSearch = apiUtility.findMobilebyId(himachalCoordsAsRO.get("email"));
...
//		                }
		        ...
		        case Constants.ROLE_CA:
					...
					String coemails = relayIPDao.convertSetToString(coordinators);
		            if (isNicEmployee) {
		        ...
		        case Constants.ROLE_SUP:
		            ...
		            if (adminType.equals("da")) {
		                correctRecpEmails = relayIPDao.isRecipientDa(toEmails,fetchAliases1);
		                if (!correctRecpEmails.isEmpty()) {
		                    correctRecpEmailsInString = relayIPDao.convertSetToString(correctRecpEmails);
		                ...
		                correctRecpEmails = relayIPDao.isRecipientAdmin(toEmails);
		                if (!correctRecpEmails.isEmpty()) {
		                    correctRecpEmailsInString = relayIPDao.convertSetToString(correctRecpEmails);
		                    forwardToData.put("nextlevel", Constants.ROLE_MAILADMIN);
		                } else {
		            	...
		            	correctRecpEmails = relayIPDao.isRecipientCoordinator(toEmails,fetchAliases1);
		                if (!correctRecpEmails.isEmpty()) {
		                    correctRecpEmailsInString = relayIPDao.convertSetToString(correctRecpEmails);
		                    if (correctRecpEmails.contains("support@gov.in") || correctRecpEmails.contains("support@nic.in") || correctRecpEmails.contains("support@dummy.nic.in")) {
		                        forwardToData.put("nextlevel", Constants.ROLE_MAILADMIN);
			...
			public Map<String, Object> fetchCoordinators(Map<String, String> form_details, String role, String uemail,
				...
				if (form_details.get("applicant_employment").equalsIgnoreCase("state")
					...
					punjabCoords = relayIPDao.fetchPunjabNodalOfficers(form_details.get("applicant_posting_city"));
					punjabDA = relayIPDao.fetchPunjabDA(form_details.get("applicant_employment"), form_details.get("applicant_ministry"),
							form_details.get("applicant_department"));
					empCoords.put("coordinators", relayIPDao.convertStringToSet(punjabCoords));
					empCoords.put("daEmail", punjabDA);
					empCoords.put("isDaAvailable", true);
				} else {
					fetchcoords = relayIPDao.fetchEmailDetails(form_details.get("applicant_employment"), form_details.get("applicant_ministry"),
							form_details.get("applicant_department"));
					...
					Set<String> finalCoords = relayIPDao.removeSupportFromSet(coordinators);

					// remove support id from admins
					Set<String> finalAdmins = relayIPDao.removeSupportFromSet(admins);
					...
					if (!finalCoords.isEmpty() && !finalAdmins.isEmpty()) {
						if (finalAdmins.equals(finalCoords)) {
							empCoords.put("isDaAvailable", true);
							empCoords.put("daEmail", relayIPDao.convertSetToString(finalAdmins));
						} else {
							...
							if (form_details.get("applicant_employment").equalsIgnoreCase("state")
								...
								if (flagForDA) {
									empCoords.put("isDaAvailable", true);
									empCoords.put("daEmail", relayIPDao.convertSetToString(da));
								}
							}
						}
					} else {
	...
	public boolean reject(String reg_no, String role, String statRemarks, String check, String email,
		...
		log.info("Inside reject method at " + new Date() +"  forwardBydata:::"+forwardBydata);
		log.info("Inside reject method at " + new Date() +"  forwardToData:::"+forwardToData);
		if (relayIPDao.updateTrackTable(forwardBydata, forwardToData)) {
	...
	public HashMap<String, Object> fetchTrackDetails(String reg_no) {
		...
		fetchDaoList=relayIPDao.fetchTrackDetails(reg_no);
		log.info("Inside fetchTrackDetails method at " + new Date() +" fetchDaoList:::"+fetchDaoList);
		List<RelayIPStatusDTO> arrRoles = (List<RelayIPStatusDTO>) fetchDaoList.get("arrRoles");
		...
		List<RelayIPFinalAuditTrackDTO> currentDetails = (List<RelayIPFinalAuditTrackDTO>) fetchDaoList.get("currentDetails");
	...
	public Map<String, Object> fetchTrackByRole(String reg_no, String forward, String trole, String srole) {
		...
		Map<String, Object> getDaoVal = relayIPDao.fetchTrackByRole(reg_no, forward, trole, srole);
	...
	public Map<String, Object> validateStatus(String stat_process,String forward,String srole,String current_user,String recv_date,String s_role,String t_role){
		log.info("Inside TrackUserService: validateStatus method at " + new Date() +" stat_process:::"+stat_process+"  forward:::"+forward+" srole:::"+srole+"  current_user:::"+current_user+"  recv_date:::"+recv_date+"  s_role:::"+s_role+"  t_role:::"+t_role);
	...
	public Map<String, String> validateRoles(List<RelayIPStatusDTO> arrRoles){
		...
		for (RelayIPStatusDTO trackStatus : arrRoles) {
	...
	public Map<String, String> validateDetails(List<RelayIPFinalAuditTrackDTO> currentDetails){
		...
		for (RelayIPFinalAuditTrackDTO trackdto : currentDetails) {
	...
	public  Map<String, Object> raiseQuery(String regid, String role, String uemail,String choose_recp,String to_email,String statRemarks) {
		...
		Map<String, Object> map=null;
		map = relayIPDao.raiseQuery(regid, role, uemail,choose_recp,to_email,statRemarks);
	...
	public ArrayList<String> fetchRecps(String regNo, String role) {
		...
		map=relayIPDao.fetchRaiseQueryData(regNo,role);
	...
	public Map<String, Object> putOnHold(String regNo, String type, String role, String statRemarks) {
		...
		Map<String, Object> hmTrack = new HashMap<>();
		 ...
		 flag = relayIPDao.putOnHold( regNo,  on_hold,  statRemarks);
         if (flag) {
	...
	public JasperPrint generatePdf(String regid) throws JRException, IOException {
		...
		RelayIPBaseDTO relayIPBaseDTO = relayIPDao.findByRegno(regid);
		if (relayIPBaseDTO!=null) {
			GeneratePdfBean generatePdfBean = new GeneratePdfBean();
			try {
				BeanUtils.copyProperties(relayIPBaseDTO, generatePdfBean);
			} catch (IllegalAccessException e) {
				e.printStackTrace();
			} catch (InvocationTargetException e) {
				e.printStackTrace();
	...
	public ArrayList<UploadMultipleFilesBean> saveDocx(UploadMultipleFilesBean uploadfiles) {
		...
		for (MultipartFile f : infile) {
			if (!f.isEmpty()) {
				...
				try {
					String outputfile = new StringBuilder().append(uploadfiles.getRegno()).append("_")
					...
					RelayIPDocUploadDTO relayipdocUploadDTO = new RelayIPDocUploadDTO();
					BeanUtils.copyProperties(uploadfiles, relayipdocUploadDTO);
					relayipdocUploadDTO.setDoc(outputfile);
					relayipdocUploadDTO.setDocpath(fileBasePath);
					relayipdocUploadDTO.setExtension(ext);
					relayipdocUploadDTO.setOriginal_filename(f.getOriginalFilename());
					relayipdocUploadDTO.setRegistrationno(registrationno);
					relayipdocUploadDTO.setRole(role);
					relayipdocUploadDTO = relayIPDao.saveDocx(relayipdocUploadDTO);
					log.info("Entering saveDocx method at info " + new Date() +" id::::"+relayipdocUploadDTO.getId());
					if (relayipdocUploadDTO.getId() == 0) {
	...
	public Map<String, Map<String, String>> viewDocx(String regid, String role) {
		...
		case "sup":
			alldocx = relayIPDao.viewDocx(regid, "sup");
			alldocx = relayIPDao.viewDocx(regid, "admin");
			alldocx = relayIPDao.viewDocx(regid, "da");
		case "admin":
		case "da":
			if (!role.equals("sup")) {
				  if (role.equals("da")) {
					  relayIPDao.viewDocx(regid, "da");
	              } else {
	            	  relayIPDao.viewDocx(regid, "admin");
	              }
				  files.putAll(alldocx);
			}
		case "co":
			alldocx = relayIPDao.viewDocx(regid, "co");
			files.putAll(alldocx);
		case "ca":
			alldocx = relayIPDao.viewDocx(regid, "ca");
			files.putAll(alldocx);
		case "user":
			alldocx = relayIPDao.viewDocx(regid, "user");
			files.putAll(alldocx);
			break;
		}
