package in.nic.eform.updatemobile.service;
...
import in.nic.eform.updatemobile.bean.MobileGeneratePdfBean;
import in.nic.eform.updatemobile.bean.MobilePreviewFormBean;
import in.nic.eform.updatemobile.bean.MobileSubmissionBean;
...
import in.nic.eform.updatemobile.dao.MobileDao;
...
public class MobileService {
	...
	public boolean validateRefNo(String refNo) {
		return mobileDao.validateRefNo(refNo);
	}
	
	
	public Map<String, String> approve(MobileForwardBean forwardBean) {
		...
		form_details = mobileDao.fetchFormDetail(forwardBean.getRef_num());
...
//                    forwardToData.put("toemail", fetchToEmail);
                    ...
                    Map<String, String> himachalCoordsAsRO = mobileDao.fetchHimachalCoord(form_details.get("department"));
                    forwardToData.put("nextlevel", Constants.ROLE_CA);
                    forwardToData.put("toemail", hod_email);
...
//						forwardToData.put("toemail", forwardDAO.fetchToEmail());
						...
						Map<String, String> himachalCoordsAsRO = mobileDao
		...
		case Constants.ROLE_CA:
			...
			String coemails = mobileDao.convertSetToString(coordinators);
			...
			fetchToEmail = mobileDao.fetchToEmail();
		...
		case Constants.ROLE_SUP:
			...
			Set<MobileEmpCoordDTO> correctRecpEmails1 = null;
			String correctRecpEmailsInString = "";
			boolean errorFlag = false;
			if (adminType.equals("da")) {
				correctRecpEmails = mobileDao.isRecipientAdmin(toEmails);
				if (!correctRecpEmails.isEmpty()) {
					correctRecpEmailsInString = mobileDao.convertSetToString(correctRecpEmails);
					forwardToData.put("nextlevel", Constants.ROLE_MAILADMIN);
				} else {
					errorFlag = true;
				}
				...
				correctRecpEmails1 = mobileDao.isRecipientCoordinator(toEmails);
				for(MobileEmpCoordDTO forwardEmpCoordDTO : correctRecpEmails1) {
				...
				if (!correctRecpEmails.isEmpty()) {
					...
					if (correctRecpEmails.contains("support@gov.in") || correctRecpEmails.contains("support@nic.in")
						...
						forwardToData.put("nextlevel", Constants.ROLE_MAILADMIN);
						correctRecpEmailsInString = mobileDao.fetchToEmail();
					} else {
						forwardToData.put("nextlevel", Constants.ROLE_CO);
					}
				} else {
		...
		case Constants.ROLE_MAILADMIN:
			...
			forwardToData.put("toemail", "");
			//update required in ldap for new mobile
			break;
		}
	...
	public boolean updateStatus(String ref_num, String role, String submissionType, String statRemarks, String file,
		...
		forwardBydata.put("formType", "mobile");
...
//			}
	...
	public int updateTrackTable(Map<String, String> forwardBydata, Map<String, String> forwardToData) {
		...
		if (role.equals(Constants.ROLE_USER)) {
			i = mobileDao.insertIntoAppType(forwardBydata, forwardToData);
			if (forwardBydata.get("check").equalsIgnoreCase("upload_scanned")) {
				j = mobileDao.updateFinalAuditTrack(forwardBydata, forwardToData);
			} else {
				j = mobileDao.insertIntoFinalAuditTrack(forwardBydata, forwardToData);
		...
		//	i = forwardDAO.updateAppType(forwardBydata, forwardToData);
		 ...
		 if (j > 0) {
				k = mobileDao.updateStatusTable(forwardBydata, forwardToData);
			}
...
//		}
	...
	public Set<String> isRecipientCoordinator(String email, String table_name) {
		...
		for (String coord : coords) {
			...
			for (String aliase : aliases) {
				Set<MobileEmpCoordDTO> correctRecpEmails1 = mobileDao.isRecipientCoordinator(aliase);
				for(MobileEmpCoordDTO forwardEmpCoordDTO : correctRecpEmails1) {
	...
	public Map<String, Object> fetchCoordinators(Map<String, String> form_details, String role, String uemail,
		...
		if (form_details.get("user_employment").equalsIgnoreCase("state")
			...
			punjabCoords = mobileDao.fetchPunjabNodalOfficers(form_details.get("applicant_posting_city"));
			punjabDA = mobileDao.fetchPunjabDA(form_details.get("user_employment"), form_details.get("min"),
					form_details.get("applicant_department"));
			empCoords.put("coordinators", mobileDao.convertStringToSet(punjabCoords));
			empCoords.put("daEmail", punjabDA);
			empCoords.put("isDaAvailable", true);
		} else {
			fetchcoords = mobileDao.fetchEmailDetails(form_details.get("user_employment"), form_details.get("min"),
			...
			Set<String> finalCoords = mobileDao.removeSupportFromSet(coordinators);

			// remove support id from admins
			Set<String> finalAdmins = mobileDao.removeSupportFromSet(admins);
			...
			if (!finalCoords.isEmpty() && !finalAdmins.isEmpty()) {
				if (finalAdmins.equals(finalCoords)) {
					empCoords.put("isDaAvailable", true);
					empCoords.put("daEmail", mobileDao.convertSetToString(finalAdmins));
				} else {
					...
					if (form_details.get("user_employment").equalsIgnoreCase("state")
						...
						if (flagForDA) {
							empCoords.put("isDaAvailable", true);
							empCoords.put("daEmail", mobileDao.convertSetToString(da));
						}
					}
				}
			} else {
	...
	public Map<String, String> validateRoles(List<MobileStatusDTO> arrRoles) {
		int i = 0;
		Map<String, String> fetchdetails = new HashMap<>();
		List<String> fetchRoles = new ArrayList<>();
		for (MobileStatusDTO trackStatus : arrRoles) {
	...
	public Map<String, String> validateDetails(List<MobileFinalAuditTrackDTO> currentDetails) {
		Map<String, String> details = new HashMap<>();
		String statusVal = "";
		String reqProcessedAs = "";
		String app_date_text = "";
		for (MobileFinalAuditTrackDTO trackdto : currentDetails) {
	...
	public JasperPrint generateFormPdf(String regid) throws JRException, IOException {
		JasperPrint jasperPrint=null;
		MobileBaseDTO generatePdfDTO = mobileDao.findByRegno(regid);
		if (generatePdfDTO!=null) {
			MobileGeneratePdfBean generatePdfBean = new MobileGeneratePdfBean();
			org.springframework.beans.BeanUtils.copyProperties(generatePdfDTO, generatePdfBean);
			File file = ResourceUtils.getFile("classpath:mobilereport.jrxml");
			...
			parameters.put("mobile", generatePdfBean.getCountry_code()+generatePdfBean.getNew_mobile());
...
//	}
	...
	public Map<String, Object> preview(String regno,String role,String applicant_email) {
		...
		System.out.println("fetchAliases::::::"+fetchAliases);
		 boolean isEditable = mobileDao.checkStatusForPreview(regno, applicant_email, role, "toPreview", fetchAliases);
		 System.out.println("isEditable::::::"+isEditable);
		  if (isEditable) {
			  map=mobileDao.preview(regno);
		  }
	...
	public Map<String, Object> EditpreviewDetails(String regno,String role,String applicant_email) {
		...
		Set<String> fetchAliases =new HashSet<String>(apiUtility.fetchAliases(applicant_email));
		System.out.println("fetchAliases::::::"+fetchAliases);
		 boolean isEditable = mobileDao.checkStatusForEdit(regno, applicant_email, role, "toEdit", fetchAliases);
		 System.out.println("isEditable::::::"+isEditable);
		  if (isEditable) {
			  map= mobileDao.EditPreview(regno);
		  }
		  System.out.println("map::::::"+map);
		  return map;
	}
	...
	public Map<String, Object> UpdatePreviewDetails(MobilePreviewFormBean previewFormBean) {
		Map<String, Object> map=new HashMap<String, Object>();
	boolean status=false;
	status= mobileDao.UpdatePreviewDetails(previewFormBean);
	...
	public Map<String, Object> putOnHold(String regNo, String type, String role, String statRemarks) {
		...
		Map<String, Object> hmTrack = new HashMap<>();
		 ...
		 flag = mobileDao.putOnHold( regNo,  on_hold,  statRemarks,role);
         if (flag) {
	...
	public  Map<String, Object> raiseQuery(String regid, String role, String uemail,String choose_recp,String to_email,String statRemarks) {
		 Map<String, Object> map=null;
		map = mobileDao.raiseQuery(regid, role, uemail,choose_recp,to_email,statRemarks);
		return map;
	}
	...
	public ArrayList<String> fetchRecps(String regNo, String role) {
		...
		map=mobileDao.fetchRaiseQueryData(regNo,role);
	...
	public boolean reject(String reg_no, String role, String statRemarks, String check, String email,
		...
		if (mobileDao.updateTrackTable(forwardBydata, forwardToData)) {
			status = true;
		}
	...
	//to view RO details if form details not pass
		public Map<String, Object> mobile()  {
			 Map<String, Object> mobileBeanList = new HashMap<String, Object>();
			 HashMap<String, Object> profileList = new HashMap<String, Object>();
			 profileList=getProfileInfo.fetchprofile();
			 mobileBeanList.put("hod_name", profileList.get("hod_name")) ;
			 mobileBeanList.put("hod_email",  profileList.get("hod_email"));
			 mobileBeanList.put("hod_mobile", profileList.get("hod_mobile"));
		return mobileBeanList;
		}
		
		public HashMap<String, Object> mobiletab2(MobileSubmissionFormBean submissionFormBean) throws IllegalAccessException, InvocationTargetException {
			HashMap<String, Object> map = new HashMap<>();
			MobileBaseDTO submissionBaseDTO = new MobileBaseDTO();
			...
			String regid = mobileDao.mobiletab2(submissionBaseDTO);
			if(regid!=null) {
				map.put("regid", regid);
				map.put("status", "ok");
				}
		...
		public Map<String, Object> consent(String regno,String check,String formtype,String consent,String email) {
			Map<String, Object> map = new HashMap<String, Object>();
			   ...
			   HashMap<String, Object> profileList = new HashMap<String, Object>();
			   MobileSubmissionBean submissionBean=new MobileSubmissionBean();
			   profileList=getProfileInfo.fetchprofile();
...
//	            session.put("admin_role", "ca");
		...
		public Map<String, String> finalSubmission(MobileSubmissionBean submissionBean) {
			Map<String, String> form_details = null;
			...
			form_details = mobileDao.fetchFormDetail(submissionBean.getRef_num());
			System.out.println("form_details::::qqqqqqqqqqqqq:::::"+form_details);
...
//		}
		...
		public HashMap<String, Object> fetchTrackDetails(String reg_no) {
			...
			fetchDaoList=mobileDao.fetchTrackDetails(reg_no);
			
			List<MobileStatusDTO> arrRoles = (List<MobileStatusDTO>) fetchDaoList.get("arrRoles");
			Map<String, String> fetchdetails =validateUsrRoles(arrRoles);
			
			List<MobileFinalAuditTrackDTO> currentDetails = (List<MobileFinalAuditTrackDTO>) fetchDaoList.get("currentDetails");
			Map<String, String> details =validateUsrDetails(currentDetails);
		...
		public Map<String, Object> fetchTrackByRole(String reg_no, String forward, String trole, String srole) {
			Map<String, Object> hmTrack = new HashMap<>();
			String reqProcessedAs = "",app_date_text="",status="";
			Map<String, Object> getDaoVal = mobileDao.fetchTrackByRole(reg_no, forward, trole, srole);
...
//		}
		...
		public Map<String, String> validateUsrRoles(List<MobileStatusDTO> arrRoles){
			int i=0;
			Map<String, String> fetchdetails = new HashMap<>();
			List<String> fetchRoles = new ArrayList<>();
			for (MobileStatusDTO trackStatus : arrRoles) {
...
//		
		...
		public Map<String, String> validateUsrDetails(List<MobileFinalAuditTrackDTO> currentDetails){
			Map<String, String> details = new HashMap<>();
			String statusVal = "";
			String reqProcessedAs = "";
			String app_date_text = "";
...
//		}
		...
		public ArrayList<MobileUploadMultipleFilesBean> saveDocx(MobileUploadMultipleFilesBean uploadfiles) {
			
		    ArrayList<MobileUploadMultipleFilesBean> list = new ArrayList<>();
	...
	String role=uploadfiles.getRole();
			MobileUploadMultipleFilesBean response;
			for (MultipartFile f : infile) {
				if (!f.isEmpty()) {
					System.out.println(registrationno+"---in service");
					...
					try {
						...
						MobileDocUploadDTO uploadfilesdto = new MobileDocUploadDTO();
						...
						uploadfilesdto = mobileDao.saveDocx(uploadfilesdto);
						System.out.println("::::::::::::" + uploadfilesdto.getId());
						if (uploadfilesdto.getId() == 0) {
						    response=new MobileUploadMultipleFilesBean();
		...
		public Map<String, Map<String, String>> viewDocx(String regid, String role) {
			...
			case "sup":
				alldocx = mobileDao.viewDocx(regid, "sup");
				alldocx = mobileDao.viewDocx(regid, "admin");
				alldocx = mobileDao.viewDocx(regid, "da");
			case "admin":
			case "da":
				if (!role.equals("sup")) {
					  if (role.equals("da")) {
						  mobileDao.viewDocx(regid, "da");
		              } else {
		            	  mobileDao.viewDocx(regid, "admin");
		              }
					  files.putAll(alldocx);
				}
			case "co":
				alldocx = mobileDao.viewDocx(regid, "co");
				files.putAll(alldocx);
			case "ca":
				alldocx = mobileDao.viewDocx(regid, "ca");
				files.putAll(alldocx);
			case "user":
				alldocx = mobileDao.viewDocx(regid, "user");
				files.putAll(alldocx);
				break;
			}
